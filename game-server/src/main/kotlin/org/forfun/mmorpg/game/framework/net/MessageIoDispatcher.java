...
import org.forfun.mmorpg.game.model.BattleMessage;
...
public class MessageIoDispatcher extends ChainedMessageDispatcher {
    ...
    public MessageIoDispatcher() {
        MessageFactory messageFactory = GameContext.messageFactory();
        ...
        MessageHandler messageHandler = (session, requestContext) -> {
            ...
            if (message instanceof BattleMessage) {
                if (GameContext.serverType == ServerType.GAME) {
                    ...
                    if (battleSid > 0) {
                        IdSession crossSession = GameContext.rpcClientRouter().getSession(battleSid);
                        if (crossSession == null) {
                            logger.error("fight server is unreachable");
                            return false;
                        }
